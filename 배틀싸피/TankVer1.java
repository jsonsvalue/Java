...
public class TankVer1 {
    ...
    private static List<int[]> bfs(boolean[][] visited, int x, int y){
    	...
    	// 나무(Grass)인지를 판별하기 위해 int 배열에 3개의 인자를 넣는다.
    	Queue<int[]> q = new LinkedList<>();
    	q.add(new int[] {x, y, 0});
    	visited[x][y] = true;
    	
    	// 각 좌표별로 내가 거쳐갔던 경로를 기록하기 위한 배열을 만든다.
    	int[][][] prev = new int[row][col][3];
    	...
    	while(!q.isEmpty()) {
    		...
    		for(int k = 0; k < 4; k++) {
    			...
    			if(!visited[nx][ny]) {
    				// 포탑까지 가는 길에, 나무가 있는지 여부를 저장한다.
    				if(mapData[nx][ny].equals("T")) {
    				...
    				// 추가로 나무를 통과하기 위해서 나무의 유무를 추가한다.
    				prev[nx][ny][0] = currX;
    				prev[nx][ny][1] = currY;
    				prev[nx][ny][2] = currTree;
    				...
    				if(mapData[nx][ny].equals("X")) {
    					...
    					while(!(pathX ==x && pathY ==y)) {
    						path.add(new int[] {pathX, pathY, pathTree});
    						
    						int prevX = prev[pathX][pathY][0];
    						int prevY = prev[pathX][pathY][1];
    						int prevTree = prev[pathX][pathY][2];
