...
public class JS {
    private static final String NICKNAME = "서울9반_살아나";
...
private static String decideAction(String[][] mapData) {
    ...
    for(int i=0; i < row; i++) {
        for(int j=0; j < col; j++) {
            ...
            if(ally[0]==0) {
            	if(mapData[i][col-j].equals("F")) {
            		secret[0] = i;
            		secret[1] = col - j;
            	}
            }else {
            	if(mapData[i][j].equals("F")) {
            		secret[0] = i;
            		secret[i] = j; 
            	}
            }
    ...
    for (String key : allies.keySet()) {
        ...
        if (key.equals("A")) {
            String mega = value[3];
            megaInt = Integer.parseInt(mega);
        }
    }
    
    //System.out.println("메가 폭탄");
    // System.out.println(megaInt);
    ...
    String command = giveAction(path, enemy, tank, megaInt, secret);
...
private static String giveAction(List<int[]> path, int[] enemy, int[] tank, int megaInt, int[] secret) {
    ...
    if( Math.abs(currX - eneX) + Math.abs(currY - eneY) <= 3 && ( currX == eneX || currY == eneY) ) {
        if(currX > nextX) {
            command = "U F";
        }else if (currX < nextX) {
            command = "D F";
    ...
    // 암호문을 해결하게 된다면, 해당되는 코드를 처리한다.
    if(Math.abs(currX - secX) + Math.abs(currY - secY) == 1) {
    	
    	StringBuilder sb = new StringBuilder();
    	for(int i = 1 ; i <= 26; i++) {
			for(int j=0; j < myCode.length(); j++) {
				char ch = myCode.charAt(j);
				int ascii = (int) ch - 65;
